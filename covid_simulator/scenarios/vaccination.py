...
from abviralsimulator.decorators import Virus, duration, showing_symphoms_after, asymptomatic, Vaccine, efficiency
...
from abviralsimulator.store import ContactCount, StateChanges, TraveledDistance
import matplotlib.pyplot as plt
from abviralsimulator import Simulator
from abviralsimulator.utility import Writer
...
class Pfizer:
    ...
    @efficiency(Covid)
    def get_covid_efficency(self, iterations_after_vacciantion, time_step):
        # 0 protetion in the first 14 days
        passed_days = (iterations_after_vacciantion * time_step) / (60 * 24)
        if passed_days < 14:
            return 0
        elif passed_days < 28:
            return 0.61
...
class Person:
    ...
    def random_walk(self, home_location, time_step):
        ...
        gyration_radius = 4 * 3800  # in meters
...
@Scenario(name="small", time_step=15, iterations=9 * 24 * 60 // 15)
class SmallScenario:

    def setup_healthcare(self):
        healthcare = PublicHealthcare(12000)

        healthcare.add_daily_count(Pfizer, 200)
...
contact_count = data_store.get_module(ContactCount)
traveled_distance = data_store.get_module(TraveledDistance)
state_changes = data_store.get_module(StateChanges)

writer = Writer(data_store)
writer.write_to_csv("vaccination")

fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2)
Plotter.draw_bar_graph(contact_count.daily_contact_count, plot=ax1,  title="Contacts per day")
Plotter.draw_bar_graph(contact_count.daily_unique_contact_count, plot=ax2, title="Unique contacts per day")
Plotter.draw_bar_graph(traveled_distance.traveled_distance, plot=ax3, title="Covered ground per day")
Plotter.draw_active_infected(state_changes.state_changes, plot=ax4, plot_days=True, title="Infected per day")
