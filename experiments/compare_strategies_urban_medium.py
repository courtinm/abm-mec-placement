...
(output/output-urban_medium/models/). Does NOT retrain anything --
...
Usage
-----
    python experiments/compare_strategies_urban_medium.py
...
# Allow `python experiments/compare_strategies_urban_medium.py` to find the
...
MODELS_DIR   = os.path.join("output", "output-urban_medium", "models")
...
def _check_models_exist(config):
    ...
    if missing:
        raise SystemExit(
            ...
            + "\n  ".join(missing)
            + "\nTrain them first, e.g.: python experiments/generate_results.py --scenarios urban_medium"
        )
