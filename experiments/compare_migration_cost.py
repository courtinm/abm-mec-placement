...
by generate_results.py (output/output-{scenario}/models/). Does NOT
...
Usage
-----
    python experiments/compare_migration_cost.py                                   # urban_medium, seed 0, all 5 strategies
    python experiments/compare_migration_cost.py --scenarios urban_medium urban_dense --seeds 0 1 2
    python experiments/compare_migration_cost.py --strategies random trained        # subset of strategies
...
# Allow `python experiments/compare_migration_cost.py` to find the
# project-root packages (main.py, agents/, configs/) regardless of the
# current directory.
...
def _check_models_exist(scenario, models_dir, config):
    ...
    if missing:
        raise SystemExit(
            ...
            f"\nTrain them first, e.g.: python experiments/generate_results.py --scenarios {scenario}"
        )
...
def main():
    ...
    for scenario in args.scenarios:
        models_dir = os.path.join("output", f"output-{scenario}", "models")
        config = importlib.import_module(f"configs.{scenario}").CONFIG
