...
generate_results.py
-------------------
Lance toutes les expériences (train + eval) et génère les figures par scénario.

Usage
-----
    python generate_results.py                         # tous les scénarios, paramètres par défaut
    python generate_results.py --scenarios urban       # un seul scénario
    python generate_results.py --steps-train 200       # mode rapide pour tester

Structure de sortie
-------------------
    resultats/
      resultats-urban_light/
      ...
      resultats-urban_medium/
      resultats-urban_dense/
"""
...
def _collect_satisfaction(eval_base, strategy, warmup):
    """Retourne la liste des taux de satisfaction moyens (post-warmup) par seed."""
    results = []
...
def _collect_timeseries(eval_base, strategy, warmup):
    """Retourne une Series step→taux_moyen sur toutes les seeds (post-warmup)."""
    strat_dir = os.path.join(eval_base, strategy)
    if not os.path.isdir(strat_dir):
        return None
    all_dfs = []
...
def run_train_rn(scenario, steps, models_dir, logs_dir):
    ...
    trend   = "↑ amélioration" if np.mean(last20) > np.mean(first20) else "→ stable"
    print(f"    récompense  premiers 20% = {np.mean(first20):.2f}  "
...
def run_train_cr(scenario, steps, models_dir, logs_dir, warmup):
    ...
    # Réinitialiser les CRs définis dans le config avant d'attacher l'agent
    ...
    if cr_rows:
        ...
        trend   = "↑ amélioration" if np.mean(last20) > np.mean(first20) else "→ stable"
        print(f"    satisfaction  premiers 20% = {np.mean(first20):.3f}  "
...
def _fig2_delta_q(logs_dir, out_dir):
    ...
    if not os.path.exists(dq_path):
        print("    [SKIP] fig2 — cr_delta_q.csv introuvable")
        return
...
def _fig3_strategy_comparison(logs_dir, out_dir, scenario, warmup):
    ...
    if not valid:
        print("    [SKIP] fig3 — pas de données eval")
        return
...
def _fig4_per_app_satisfaction(logs_dir, out_dir, warmup):
    ...
    if not data:
        print("    [SKIP] fig4 — pas de données")
        return
...
def _fig5_satisfaction_timeseries(logs_dir, out_dir, warmup):
    ...
    if not plotted:
        print("    [SKIP] fig5 — pas de données eval")
        plt.close(fig)
        return
...
def main():
    parser = argparse.ArgumentParser(
        description="Lance toutes les expériences et génère les figures par scénario."
    )
    parser.add_argument(
        "--scenarios", nargs="+", default=SCENARIOS_DEFAULT,
        help=f"Scénarios à traiter (défaut: {SCENARIOS_DEFAULT})"
    )
    parser.add_argument(
        "--steps-train", type=int, default=TRAIN_RN_STEPS,
        help=f"Steps d'entraînement RN (défaut: {TRAIN_RN_STEPS}). "
             f"CR steps: {TRAIN_CR_STEPS} (valeurs par scénario, non modifiables via CLI)"
    )
    parser.add_argument(
        "--steps-eval", type=int, default=EVAL_STEPS,
        help=f"Steps d'évaluation par run (défaut: {EVAL_STEPS})"
    )
    parser.add_argument(
        "--seeds", type=int, default=len(SEEDS_DEFAULT),
        help=f"Nombre de seeds pour l'évaluation (défaut: {len(SEEDS_DEFAULT)})"
    )
    parser.add_argument(
        "--skip-train", action="store_true",
        help="Sauter l'entraînement (réutiliser les modèles existants)"
    )
    parser.add_argument(
    ...
    print(f"\n{'='*62}")
    print("  Terminé. Résultats dans resultats/")
    print("=" * 62)
