...
class CRPlacementAgent:
    ...
    _LOW_MAX = 3  # demand level 1 ("low") covers 1-3 users; 4+ is level 2 ("high")
    ...
    def __init__(self, candidate_bs, k, cr_capacity_mbps,
        ...
        # State-visit counts and greedy-policy snapshots, used by
        # run_experiment.py's --state-coverage-every / --snapshot-every
        # (see EXPERIMENTS.md, Stage 7).
        self.state_visit_counts = {}
        self.policy_snapshot_history = []
    ...
    def select_action(self, state):
        ...
        # Counted here (not in update()) because this is the only point where
        # `state` is actually acted upon; update() also inserts next_state
        # look-aheads that may never be visited as a real decision.
    ...
    def capture_policy_snapshot(self, min_visits):
        """
        Record the greedy action for every state visited >= min_visits times.

        Restricted to well-visited states so the snapshot reflects a settled
        policy rather than noise from a few early, mostly-random updates.
        """
        snapshot = {
            state: max(actions, key=actions.get)
            for state, actions in self.q_table.items()
            if self.state_visit_counts.get(state, 0) >= min_visits
        }
        self.policy_snapshot_history.append(snapshot)
