...
Usage
-----
    python analysis/factorial_analysis.py --scenarios urban_light --strategies trained
    python analysis/factorial_analysis.py                                   # all 3 scenarios, all 5 strategies

(always run from the project root, abm_communication_networks/)

Requires eval logs for all 4 conditions to already exist:
    M0: output/output-{scenario}/logs/eval/{strategy}/s{seed}/satisfaction_summary.csv
    M1/M2/M3: output/output-{scenario}__M{1,2,3}/logs/eval/{strategy}/s{seed}/...
...
def _eval_dir(scenario, condition):
    ...
    return os.path.join("output", f"output-{scenario}{suffix}", "logs", "eval")
